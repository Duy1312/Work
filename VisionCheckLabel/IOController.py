...
class IOController:
    def __init__(self, port_name: str, baud_rate: int = 19200, io_type: IOType = IOType.EightPorts):
    ...
    def add_data_received_callback(self, callback: Callable[[object, IODataReceivedEventArgs], None]):
    ...
    def write_out(self, command: OutPorts, state: PortState) -> bool:
        ...
        try:
            bytes_to_send = bytearray([
                0x98,
                self.port_out[command.value],
                state.value,
                0x99
            ])
            self.write_line(f"Sending raw data: {' '.join([f'{b:02X}' for b in bytes_to_send])}")
    ...
    def read_loop(self):
        ...
        while self.running and self.is_open():
            try:
                ...
                if self.serial_port.in_waiting > 0:  # Thay đổi để đọc bất kỳ dữ liệu nào
                    # Read available bytes
                    buffer = self.serial_port.read(self.serial_port.in_waiting)
                    self.write_line(f"Raw data received: {' '.join([f'{b:02X}' for b in buffer])}")
            ...
            while self.running and self.is_open():
                try:
                    ...
                    if self.serial_port.in_waiting >= self.received_bytes_threshold:
                        ...
                        debug_str = "Received bytes: " + " ".join([f"{b:02X}" for b in buffer])
    ...
    def process_in_data(self, buffer: bytes):
        ...
        temp = self.stored_byte_in - buffer[1] if buffer[1] < self.stored_byte_in else buffer[1] - self.stored_byte_in
...
class IOControllerApp:
    ...
    def create_gui(self):
        ...
        # IO Type selection
        ttk.Label(conn_frame, text="Loại I/O:").grid(row=0, column=2, padx=5, pady=5, sticky=tk.W)
        self.io_type_var = tk.StringVar(value="EightPorts")
        ttk.Combobox(conn_frame, textvariable=self.io_type_var, values=["FourPorts", "EightPorts"], width=10, state="readonly").grid(row=0, column=3, padx=5, pady=5, sticky=tk.W)
        ...
        self.disconnect_btn = ttk.Button(conn_frame, text="Ngắt kết nối", command=self.disconnect, state=tk.DISABLED)
        ...
        for i in range(1, 9):
            ...
            indicator = ttk.Label(frame, text="●", foreground="gray", font=("Arial", 16))
        ...
        ttk.Button(all_ports_frame, text="Bật tất cả", 
                  command=lambda: self.send_output(0, PortState.On)).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(all_ports_frame, text="Tắt tất cả", 
                  command=lambda: self.send_output(0, PortState.Off)).pack(side=tk.LEFT, padx=5)
    ...
    def connect(self):
        ...
        io_type = IOType.EightPorts if self.io_type_var.get() == "EightPorts" else IOType.FourPorts
