...
class Canvas(QLabel):
    ...
    def __init__(self, parent=None, bcontext_menu=True, benable_drawing=True):
        ...
        self.label_pos.setStyleSheet("QLabel{background-color:rgba(128, 128, 128, 150); color:white; font:bold 12px}")
        self.label_pos.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        self.zoom_buttons = newDialogButton(self.tool_zoom, ["", "", "", ""], 
                        [lambda:self.zoom_manual(1.2), lambda:self.zoom_manual(0.8), lambda:self.fit_window(), self.on_show_full_screen],
                        icons=["zoom_in", "zoom_out", "zoom_fit", "full_screen"],
                        orient=Qt.Orientation.Horizontal).buttons()
